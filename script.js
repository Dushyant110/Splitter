        ...
        function calculatePayments() {
            ...
            for (let i = 0; i < payments.length - 1; i++) {
                for (let j = i + 1; j < payments.length; j++) {
                    const payer = payments[i];
                    const receiver = payments[j];
                    const difference = averageExpense - (payer.personTotal + receiver.personTotal) / 2;
                    const paymentTo = difference.toFixed(2);
                    const paymentInfo = document.createElement('p');
                    paymentInfo.textContent = `${payer.name} should pay ${paymentTo} to ${receiver.name}.`;
